...
load("../module2/environment.RData")
...
plot(roc(test_y, y_hat.ss),
  print.auc = TRUE, col = 'green', add = TRUE, print.auc.y = 0.3
)
legend(0, 0.2, legend = c("LASSO", "ALASSO", "Two-Step"), col = c("black","red", "green"), 
       lty = 1, cex = 0.8)
```

```{r}
FPR = 0.05
rbind(coords(roc = roc(test_y, y_hat.ss), x = FPR, input = "fpr")[-1])
```
...
```{r}
start<- Sys.time()
auc_twostep <- validate_ss(dat = labeled_data, nsim = 600, 
                    n.train = c(50, 70, 90),
                    beta = beta.step1,
                    S = S,
                    x = x)
...
```
```{r}
boxplot(auc_twostep, ylim = c(0.5, 1), names = c("n=50", "n=70", "n=90"),
        main = "Two-Step Semi-Supervised")
```
```{r}
# Compare with Previous method
boxplot(cbind(auc_supervised, auc_twostep) %>% select(starts_with("n=50")), 
        ylim = c(0.5, 1), names = c("LASSO", "ALASSO", "Two-Step"), main = "n=50")
boxplot(cbind(auc_supervised, auc_twostep) %>% select(starts_with("n=70")), 
        ylim = c(0.5, 1), names = c("LASSO", "ALASSO", "Two-Step"), main = "n=70")
boxplot(cbind(auc_supervised, auc_twostep) %>% select(starts_with("n=90")), 
        ylim = c(0.5, 1), names = c("LASSO", "ALASSO", "Two-Step"), main = "n=90")
```
...
plot(roc(test_y, y_hat.phenorm),
  print.auc = TRUE, col = 'orange', add = TRUE, print.auc.y = 0.2
)

legend(0, 0.4, legend = c("LASSO", "ALASSO", "Two-Step", "PheNorm"), 
...
plot(roc(test_y, y_hat.map),
  print.auc = TRUE, col = 'orchid', add = TRUE, print.auc.y = 0.1
)

legend(0, 0.4, legend = c("LASSO", "ALASSO", "Two-Step", "PheNorm", "MAP"), 
       col = c("black","red", "green","orange","orchid"), 
       lty = 1, cex = 0.8)
```