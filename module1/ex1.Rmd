...
```{r}
packages <- c(
	  "tidyverse", "PheCAP", "glmnet", "glmpath",
	  "corrplot", "mclust", "corrplot", "ggplot2"
)
```
...
More healthcare utilization, more featuer counts; but not necessary the patient get the disease.  
...
Different models to fit the clustering. Try `?Mclust()`.

Assume the multivariate mixture follows diagonal, equal volume and shape. "EEI".

```{r}
model_clust_EEI <- clustering_method(s = cbind(sicd, snlp), x = x, mclust_model = "EEI")
Cluster_select_EEI <- model_clust_EEI$beta_select
Cluster_feature_EEI <- colnames(x)[Cluster_select_EEI]
Cluster_feature_EEI
```
...
Save the data and feature selected for module 2 and model fitting. 

```{r}
save(list = ls(), file = "../module2/ex1Environment.RData")
```
